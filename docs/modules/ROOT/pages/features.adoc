...
A live command preview shows the exact `jbang init` invocation, with a *Copy* button that copies it to the clipboard.
Destinations are validated before creation: existing files, absolute paths, and paths that escape the selected directory are rejected.
